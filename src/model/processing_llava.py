...
class LlavaProcessor(ProcessorMixin):
    ...
    def __call__(self, text, images=None, 
        ...
        if image_path_in_tag and not images:
            ...
            for i, conversation in enumerate(text): # for each coversation in the batch
                ...
                for n, message in enumerate(conversation): # for each message in the conversation
                    if message['role'] == "user": # if the message is from the user, then we can check the image tags
                        ...
                        if image_paths: # if found the image path in the image tag
                            text[i][n]["content"] = self.img_tag_pattern.sub("<image>", message['content'])
        ...
        ###############################################################
        image_pos = []
        image_tag = self.tokenizer.image_token if hasattr(self.tokenizer, "image_token") else self.tokenizer.init_kwargs['image_token']
